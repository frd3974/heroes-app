    ...
    return (
        <Card className="group overflow-hidden hover:shadow-xl transition-all duration-300 hover:-translate-y-1 bg-gradient-to-br from-white to-gray-50">
            <div className="relative h-64">
                ...
                {/* Favorite button */}
                <Button onClick={() => toggleFavorite(heroe)} size="sm" variant="ghost" className="absolute bottom-3 right-3 bg-white/90 hover:bg-white">
                    <Heart className={`h-4 w-4 ${isFavorite(heroe) ? 'fill-red-500 text-red-500' : ''}`} />
                </Button>
            ...
            <CardContent className="space-y-4">
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-1">
                        {
                            heroe && heroe.powers?.map(power => (
                                <Badge key={`${power}_${heroe.id}`} variant="outline" className="text-xs">
                                    {power}
                                </Badge>
                            ))
                        }
                ...
                <div className="text-xs text-gray-500 pt-2 border-t">First appeared: {heroe.firstAppearance}</div>
            </CardContent>
        </Card>
    );
};